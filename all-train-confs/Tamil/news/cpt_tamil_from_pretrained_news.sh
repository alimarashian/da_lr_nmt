...
MODEL_TYPE=just_bible_cpt_tamil_from_pretrained_news
TGT_LANG=tm_XX
IS_LECA=false
DICTIONARY_PATH=/projects/sema4648/mb_fairseq/mbart-bible/dictionaries/Tamil/news/en-tm-news-merged.txt ## this are basically placeholders when doing the basic mbart, so doesn't have to match
LEMMAS_PATH=/projects/sema4648/mb_fairseq/mbart-bible/dictionaries/Tamil/news/all_lemmas_Tamil_dictionary_news.txt
MAX_EPOCH=150
MIN_EPOCH=75
EARLY_STOPPING=true
PATIENCE=50
...
FROM_PRETRAINED_NAME=pretrained_liu_tamil_news
...
if [ $FROM_PRETRAINED == false ]; then
    ...
    MBART_MODEL=/rc_scratch/sema4648/project_data/bible_data/$TARGET_LANG_NAME/trained_models/$FROM_PRETRAINED_NAME/checkpoint_best.pt
    LOAD_FROM_PRETRAINED="--been-cont-pretrained"
else
    MBART_MODEL=/rc_scratch/sema4648/project_data/$FROM_PRETRAINED_DOMAIN/$TARGET_LANG_NAME/pretrained_models/$FROM_PRETRAINED_NAME/checkpoint_best.pt
    LOAD_FROM_PRETRAINED="--been-cont-pretrained"
fi
