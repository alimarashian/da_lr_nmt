...
# python $SCRIPT_PATH/split_raw-1.py $DOMAIN $TARGET_LANG_NAME $SRC_CODE $TGT_CODE $SETTING $DO_PRETRAIN $DO_TRAIN $DO_TEST $PRETRAIN_NUM $TEST_SIZE $SEED $MAIN_DATA_PREPROCESS_PATH
# exit
...
# the english and target language bible data should be in the folder '$MAIN_DATA_PREPROCESS_PATH/bible_data/$TARGET_LANG_NAME' as files bible.en and bible.TGT_CODE
if [ $MAKE_BIBLE == true ]; then
    python $BIBLE_SCRIPT_PATH $SRC_CODE $TGT_CODE $BIBLE_DATA_PATH $TARGET_LANG_NAME $VALID_PORTION $SEED
fi
...
if [ $DO_PSEUDO == true ]; then
    python  $SCRIPT_PATH/make_pseudo-2.py $DICTIONARY_PATH $SRC_CODE $TGT_CODE $DOMAIN $TARGET_LANG_NAME $MAIN_DATA_PREPROCESS_PATH
fi
...
python $SCRIPT_PATH/split_train-3.py $SRC_CODE $TGT_CODE $DOMAIN $TARGET_LANG_NAME $BIBLE_DATA_PATH $SEED $MAIN_DATA_PREPROCESS_PATH
